...
func renderWithContext(ctx context.Context, w writeCloser, index *Index, generator CommandGenerator, start time.Time, resultMeta ResultMetadata) (int, error) {
	count := 0
	lineCount := 0
	var lastName string
...
func renderSummary(ctx context.Context, w writeCloser, index *Index, generator CommandGenerator, start time.Time, resultMeta ResultMetadata) (int, error) {
	count := 0
	currentLines := 0
	var lastName string
	bw := bufio.NewWriterSize(w, 256*1024)
